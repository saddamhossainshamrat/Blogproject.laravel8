...
class FrontEndController extends Controller
{
 public function home(){
    ...
    return view('website.home', compact(['posts', 'recentPosts', 'firstPosts2','middlePost','lastPost','firstfooterPosts',
  'firstfooterPosts2','lastfooterPosts']));
 }
  ...
  public function tag($slug){
    ...
    public function post($slug){
     ...
     $lastRelatedPosts = $relatedPosts->splice(0,1);
        ...
        $tags = Tag::all();
        if($post){
        return view('website.post', compact(['post', 'posts', 'categories', 'tags', 'firstRelatedPosts','firstRelatedPosts2','lastRelatedPosts']));
        }else{
          return redirect('/');
        }
    }
